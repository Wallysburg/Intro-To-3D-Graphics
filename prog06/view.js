...
window.onload = function init()
{
	...
	for (var i =0; i <cube.verts.length ; i++) {
	    var x = cube.verts[i];
	    var y = clip2canvas.times(proj2clip.times(view2proj.times(world2view.times(rotated2world.times(object2rotated.times(model2object.times(x))))))).homogeneous();
	    y[2] = 0;
	    if(mouseCanvas.distance(y) < vertexClickDistance) {
		clickedModel = x;
	    }  
	}
    ...
    canvas.addEventListener("mousemove", function (e) {
	    ...
	    var front = new PV(cursorX, cursorY, -1, true);
	    var back = new PV(cursorX, cursorY, 1, true);
            // Transform them to f and b in the *rotated* frame.
	    var f = world2rotated.times(view2world.times(proj2view.times(clip2proj.times(canvas2clip.times(front))))).homogeneous();
	    var b = world2rotated.times(view2world.times(proj2view.times(clip2proj.times(canvas2clip.times(back))))).homogeneous();
	    ...
	    var w = vv.minus(u.times(u.dot(vv)));
	    console.log("w is : " + w);
		...
		var t = Math.sqrt(t2);
		var wplus = w.plus(u.times(t));
		var wminus = w.minus(u.times(t));
		if (wplus.distance(v) < wminus.distance(v)) {
		    w = wplus;
		}else {
		    w = wminus;
		}
	    ...
	    var vx = v.unit(); //vUnit
	    var vz = v.cross(w).unit(); //vCrosW
	    var vy = vz.cross(vx); // VCrosswCross
	    var wx = w.unit(); //wUnit
	    var wz = vz;
	    var wy = wz.cross(wx);
	    var v = new Mat(vx, vy, vz);
	    var w = new Mat(wx, wy, wz);
	    var last = w.times(v.transpose());
	    object2rotated = last.times(object2rotated);
	    rotated2object = object2rotated.times(last.transpose());
